...
Inicia un repositorio llamado CC3S2 y dentro una carpeta llamada Actividades. Dentro de esta carpeta abre una carpeta llamada Refactorizacion y coloca todas tus respuestas.
...
Hay muchas técnicas de refactorización de código y presentamos algunas de las más comunes y útiles vistas en una actividad anterior. Tu trabajo es encontrar las soluciones y como se pueden refactorizar.
...
Tú incorporas la refactorización en el ciclo de desarrollo basado en pruebas comenzando con una prueba `roja` fallida, escribiendo el código más simple posible para que la prueba pase a `verde` y finalmente trabajes para mejorar y mejorar tu código mientras mantienes la prueba `verde`. 
...
Usa palabras: `when`, `then`. Para verificar si el método devuelve `True`, utilizaremos el método `assertTrue`. 
...
Es por eso que debes escribir una prueba unitaria. Cuando cambia el código, puede revisar si todavía funciona como antes. 
